...
const Inventory = () => {
  const columns = ['#', 'Add Date', 'Product', 'Vendor', 'Customer', 'Purchase', 'Sales', 'Balance'];

  return (
    <div className="p-6 mt-14 sm:ml-64">
      <div className="pb-2 border-b border-gray-200 flex justify-between items-center">
        <span className="self-center text-xl font-semibold sm:text-2xl whitespace-nowrap">Inventory</span>
      ...
      <div className="mt-6 max-sm:mt-8 flex justify-between items-center flex-wrap">
        ...
        <div className="sm:inline-flex max-sm:-mt-6 sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
          ...
          <input type="text" id="Search" className="py-3 px-4 block w-full border border-gray-400 rounded-sm text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" placeholder="Search Inventory" />
        </div>
      </div>
      <Table data={inventoryData} columns={columns} />
